    ...
    public class SSRSAuthorization : IAuthorizationExtension
    {
        ...
        private bool HasUnRestrictedAccess(string userName)
        {
            ...
            if (0 == String.Compare(userName, m_adminUserName, true, CultureInfo.CurrentCulture))
                return true; // <--- if you sign in as IT, it will return true and give access

            // All others Are Not Unstricted
            return false; // <--- not logged in as IT, so it will return false
        }
        ...
        private bool UserMatchesPrincipal(string acePrincipal, IntPtr userToken)
        {
            ...
            return myIdentity.HasClaim(c => (c.Type == ClaimTypes.Name || c.Type == "groups" || c.Type == ClaimTypes.GroupSid || c.Type == ClaimTypes.Email || c.Type == ClaimTypes.Role) && c.Value.Equals(acePrincipal, StringComparison.OrdinalIgnoreCase));
        }
        ...
        public StringCollection GetPermissions(string userName, IntPtr userToken, SecurityItemType itemType, byte[] secDesc)
        {
            ...
            { // attempt to deserialize acl list. But notice that secDesc is empty. Not sure what that is supposed to be? So it wil have no ACLs
                AceCollection acl = DeserializeAcl(secDesc);
                foreach (AceStruct ace in acl)
                {
                    if (UserMatchesPrincipal(ace.PrincipalName, userToken))
